...
public class PlotIntermediateFrequencyPart implements Runnable{
    private static XYSeries xyseries = new XYSeries("Frequency_i");
    private static XYSeries xyseries2 = new XYSeries("Frequency_q");
    private static JFreeChart jfreechart = null;
    public boolean isRunning=true;
    private LinkedBlockingQueue queue;
    private JPanel parent;
    private static int[] message;
    public PlotIntermediateFrequencyPart(LinkedBlockingQueue queue,JPanel parent) {
        this.parent = parent;
        this.queue=queue;
        parent.add(PlotIntermediateFrequencyPart.getJFreeChart());
    }
    public static JPanel getJFreeChart(){
        ...
        jfreechart.setTitle("中频信号时域图");
        XYPlot xyplot = (XYPlot) jfreechart.getPlot();
        // Y轴
        NumberAxis numberaxis = (NumberAxis) xyplot.getRangeAxis();
        numberaxis.setLowerBound(-2047*0.3);
        numberaxis.setUpperBound(2047*0.3);
        numberaxis.setTickUnit(new NumberTickUnit(200d));
        numberaxis.setLabel("Magnitude/mV");
        ...
        x.setTickUnit(new NumberTickUnit(1000d));
        // 设置最大的显示值和最小的显示值
        x.setLowerBound(0);
        x.setUpperBound(5000);
        // 数据轴的数据标签：只显示整数标签
    ...
    public void run() {
        ...
        while (true) {
            try {
                ...
                while (isRunning) {
                    xyseries.addOrUpdate(i, message[0]*0.3);
                    xyseries2.addOrUpdate(i,message[1]*0.3);
                    //System.out.println("out="+message[0]+","+message[1]);
                    i++;
                    Thread.sleep(0);
                    if (i == 5000) {
                        xyseries.delete(0, 4999);
                        xyseries2.delete(0,4999);
                        i = 0;
                        continue;
                    }
                    break;
